...
var mtCookieName = "mt_blog29_user";
var mtCookieDomain = ".blog.linux.org.tw";
var mtCookiePath = "/~fourdollars/";
...
function mtShowCaptcha() {
    ...
    if (div) {
        div.innerHTML = '<div class="label"><label for="captcha_code">Captcha:</label></div><div class="field"><input type="hidden" name="token" value="VRbgD414oiGM6nnhT8c3rgxcZpi0m5AMS8KfE58k" /><img src="/cgi-bin/movabletype/mt-comments.cgi/captcha/29/VRbgD414oiGM6nnhT8c3rgxcZpi0m5AMS8KfE58k" width="150" height="35" /><br /><input name="captcha_code" id="captcha_code" value="" autocomplete="off" /><p>Type the characters you see in the picture above.</p></div>';
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://blog.linux.org.tw/cgi-bin/movabletype/mt-comments.cgi?__mode=session_js&blog_id=29&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '28ca05961ad698354ead34a1db5930f2eeb6c39f';
...
function mtSignIn() {
    ...
    var url = 'http://blog.linux.org.tw/cgi-bin/movabletype/mt-comments.cgi?__mode=login&blog_id=29';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fblog.linux.org.tw%2F~fourdollars%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://blog.linux.org.tw/cgi-bin/movabletype/mt-comments.cgi?__mode=handle_sign_in&static=0&logout=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://blog.linux.org.tw/cgi-bin/movabletype/mt-comments.cgi?__mode=edit_profile&return_url=' + encodeURIComponent( location.href );
                user_link += '">' + u.name + '</a>';
            } else {
